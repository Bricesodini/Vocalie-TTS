...
def test_run7_dynamic_preview_changes_duration(monkeypatch, tmp_path):
    result = _setup_session(monkeypatch, tmp_path, text="Bonjour, salut.\nMerci.")
...
def test_run7_apply_creates_processed_v2(monkeypatch, tmp_path):
    result = _setup_session(monkeypatch, tmp_path, text="Bonjour, salut.\nMerci.")
    session_dir = Path(result[-1]["dir"])
    edits = [{"type": "INSERT_SILENCE", "at_s": 0.0, "duration_ms": 200, "snap": "off"}]
    ...
    assert "v2" in (session_data.get("takes", {}).get("processed") or [])
import pytest

pytest.skip("Dynamic edit preview removed in V2.", allow_module_level=True)
