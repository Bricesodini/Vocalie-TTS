...
- Path constants (`WORK_DIR`, `OUTPUT_DIR`, `PRESETS_DIR`): `backend/config.py` — the single source of truth; root shims and `backend/` modules both import from `backend.config`.
